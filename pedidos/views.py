...
import logging
from django.contrib.auth.models import User  # Añadir esta línea
from clientes.models import Cliente  # Add this import at the top
...
# Eliminar o comentar esta función por completo
# @background(schedule=20)
# def eliminar_pedido_no_efectuado(pedido_id):
#     ...

#@background(schedule=20)  # 20 segundos
def cancelar_pedido_no_efectuado(pedido_id):
    try:
        ...
        if pedido.estado == 'pendiente':
            ...
            print(f"Pedido {pedido_id} cancelado automáticamente después de 20 segundos")
    except Pedido.DoesNotExist:
        print(f"Pedido {pedido_id} no encontrado")
    except Exception as e:
        print(f"Error al cancelar pedido {pedido_id}: {str(e)}")
...
def lista_pedidos(request):
    ...
    context = {
        ...
        'gestores': User.objects.filter(is_active=True).values('id', 'first_name', 'last_name', 'username').distinct(),
